...
=== Extra ===
    - {npc_nick}_{npc_role_m}: Badan Eksekutif Mahasiswa (BEM) Universitas Brawijaya merupakan sebuah lembaga eksekutif tingkat fakultas. Membawa istilah eksekutif, BEM FILKOM bertugas untuk menjalankan program-program kerja yang selaras dan mematuhi segala tata tertib yang telah ditetapkan oleh Dewan Perwakilan Mahasiswa (DPM), yang merupakan lembaga legislatif tingkat universitas. 
    - {npc_nick}_{npc_role_m}: Dalam pendekatan kelembagaan, BEM FILKOM hadir untuk mempertanggungjawabkan hak yang seharusnya diterima oleh mahasiswa melalui kegiatan aktualasi diri kemahasiswaan. 
    - {npc_nick}_{npc_role_m}: Sederhananya, setiap uang yang dibayarkan oleh mahasiswa kepada pihak fakultas termuat hak mahasiswa dalam kegiatan aktualisasi diri kehidupan kampus. Dalam level fakultas, BEM FILKOM hadir sebagai sarana untuk mempertanggungjawabkan hal tersebut.
